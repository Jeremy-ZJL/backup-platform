...
class process_control(metaclass=ABCMeta):
    """
    抽象基类abc.abstractmethod，
    父类只是抽象方法，并不实现功能；只能继承，而不能被实例化；
    子类只有实现了父类的方法才能被实例化，否则会报错；
    放在这里是防止少写方法。
    """
    @abstractmethod
    def start(self):
...
class nginx_process_control(process_control):
    ...
    def start(self):
        state, output = subprocess.getstatusoutput(self.nginx_process)
        if state != 0:
            s = 'Nginx启动失败! 进程:%s err:%s\n' % (self.nginx_process, output)
            print("nginx's state: %s" % state)
            print(s)
        else:
...
class supervisord_process_control(process_control):
    ...
    def start(self):
        cmd = '{supervisord_env} -c {supervisord_conf}'.format(supervisord_env=self.supervisord_env,
                                                               supervisord_conf=self.supervisord_config_file)
