...
class filesystem_backup(metaclass=ABCMeta):
    ...
    def sftp_sersync_config(self):
        ...
        sftp_config_file = self.sshObj.sftpFile(sersync_config,
                                                "/usr/local/sersync/config/%s" % sersync_config.split('/')[-1], "push")
        try:
            ...
            output = self.sshObj.exeCommand(
                "ps -ef | grep %s | grep -v grep | awk -F ' ' '{print $2}'" % (sersync_config.split('/')[-1]))
            print(output, 'output')
...
class distribute_filesystem_backup(filesystem_backup):
        ...
        try:
            for j in string.splitlines():
                if j.isspace():
                    continue
                cmd = "sed -i /%s/d %s" % (
                    j.replace('[', '\\\\[').replace(']', '\\\\]').replace('/', '\\\\/'), self.rsync_config_file)
    ...
    def fs_full_backup(self, backup_to_local_path=None, fs_full_backup_path=None):
        ...
        try:
            ...
            logger.warn(
                "主机%s全量备份%s至%s失败! err:%s" % (self.source_addr, backup_to_local_path, fs_full_backup_path, str(e)))
            raise ValueError(
                "主机%s全量备份%s至%s失败! err:%s" % (self.source_addr, backup_to_local_path, fs_full_backup_path, str(e)))
    ...
    def fs_backup_start(self):
        ...
        if all([self.sftp_sersync_config(), self.mk_rsync_config(), self.rsync_start()]):
            ...
            logger.warn(
                "主机%s全量备份%s至%s失败! err:%s" % (self.source_addr, self.backup_src_path, self.backup_dst_path, str(e)))
            raise ValueError(
                "主机%s全量备份%s至%s失败! err:%s" % (self.source_addr, self.backup_src_path, self.backup_dst_path, str(e)))
