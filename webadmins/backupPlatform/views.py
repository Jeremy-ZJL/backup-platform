        ...
        try:
            db_bk_task = db.select_database(PROJ_DB_CONFIG["database"]).select_table("database_backup_task"). \
                where({"source_addr": source_addr,
                       "backup_to_local_path": os.path.join(backup_to_local_path, 'backup_mysql')}).select("*")
            if db_bk_task:
                result["code"] = 404
...
class backup_fs_manager(View):
    def get(self, request):
        ...
        try:
            if any([source_addr, backup_path]):
                msg = db.select_database(PROJ_DB_CONFIG["database"]).select_table("filesystem_backup_task").where(
                    {"source_addr": source_addr, "backup_path": backup_path}). \
                    order(order=["stat_time"]).select("*", final="dict")
            else:
    ...
    def post(self, request):
        ...
        backup_to_local_path = os.path.join(
            os.path.join(os.path.join(backup_to_local_path, 'backup_filesystem'), source_addr), backup_path.lstrip('/'))
        print(backup_to_local_path)
    ...
    def delete(self, request):
        ...
        try:
            ...
            try:
                db.select_database(PROJ_DB_CONFIG["database"]). \
                    select_table("filesystem_backup_task").where({
